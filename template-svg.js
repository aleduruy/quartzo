const { getProps, getForwardRef, getExportName } = require('@svgr/core');
...
const reactNativeTemplate = (code, config, state) => {
  const { reactNativeSvgReplacedComponents = new Set(), unsupportedComponents = new Set() } = state;
  ...
  result += warnLog;
  result += `const ${state.componentName} = ${props} => ${code.replace('"inherit"', '{props.fill}')}\n\n`;
...
module.exports = reactNativeTemplate;
